...
# Locate ProteinMPNN.  Search order:
#   1. PROTEINMPNN_DIR environment variable
#   2. ProteinMPNN/ next to this repo (sibling of the sugarfix folder)
#   3. ProteinMPNN/ inside the sugarfix folder (Colab clone location)
SCRIPT_DIR = Path(__file__).resolve().parent
REPO_ROOT = SCRIPT_DIR.parent  # sugarfix/
PMPNN_DIR = None
...
SEQUON_REGEX = re.compile(r"N[^PX][ST]")
...
def is_functional_sequon(triplet):
    """Check if a triplet is a valid N-X-S/T sequon (X != P, X != unresolved)."""
